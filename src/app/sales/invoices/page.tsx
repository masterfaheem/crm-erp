...
function InvoicesMain() {
  ...
  return (
    <main className={`min-h-screen bg-gray-50 pt-16 transition-[padding] duration-200 ease-out ${collapsed ? "lg:pl-[72px]" : "lg:pl-64"}`}>
      <div className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8"><InvoicesContent /></div>
    </main>
  );
}
...
function InvoicesContent() {
  ...
  const fetchRows = useCallback(async () => {
    ...
    try {
      ...
      setTotalRows(j.total_invoices || 0);
  ...
  const handleDelete = async (r: InvoiceRow) => {
    ...
    try {
      const res = await fetch(`/api/invoices/${r.id}`, { method: "DELETE", credentials: "include" });
    ...
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-0.5 text-sm text-gray-500">Issue customer invoices and track payment status.</p>
        </div>
        <div className="text-xs text-gray-500">{totalRows.toLocaleString()} total invoices</div>
      </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead>
              <tr className="bg-gray-50">
                ...
                <th className="hidden px-4 py-3 text-left text-xs font-medium uppercase tracking-wide text-gray-500 md:table-cell">Due</th>
              ...
              {loading ? Array.from({ length: 4 }).map((_, i) => (
                ...
                <tr><td colSpan={9} className="px-4 py-12 text-center">
                  <p className="text-sm text-gray-500">
                    {hasFilters ? "No invoices match your filters." : "No invoices yet. Create one to get started."}
                  </p>
                </td></tr>
              ) : rows.map((r) => (
                <tr key={r.id} className="hover:bg-gray-50">
                  <td className="whitespace-nowrap px-4 py-3.5">
                    <span className="font-mono text-sm font-medium text-gray-900">{r.invoice_number}</span>
                    {r.so_number && <div className="text-[11px] text-gray-400">SO: {r.so_number}</div>}
                  </td>
                  <td className="px-4 py-3.5 text-sm text-gray-700">{r.customer_name}</td>
                  <td className="hidden whitespace-nowrap px-4 py-3.5 text-sm text-gray-600 md:table-cell">{formatDate(r.invoice_date)}</td>
                  <td className="hidden whitespace-nowrap px-4 py-3.5 text-sm text-gray-600 md:table-cell">{formatDate(r.due_date)}</td>
                  ...
                  <td className="hidden whitespace-nowrap px-4 py-3.5 text-right text-sm text-gray-600 lg:table-cell">{formatCurrency(Number(r.paid_amount))}</td>
                  <td className="hidden whitespace-nowrap px-4 py-3.5 text-right text-sm lg:table-cell">
                    <span className={Number(r.balance) > 0 ? "font-medium text-red-600" : "text-gray-500"}>
      ...
      {showModal && (
        <InvoiceModal invoice={editing} customers={customers}
...
function InvoiceModal({
  ...
  const [invoiceDate, setInvoiceDate] = useState<string>(invoice?.invoice_date?.slice(0, 10) || new Date().toISOString().slice(0, 10));
        ...
        <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
          <div>
            ...
            <p className="mt-0.5 text-xs text-gray-500">
              {isEdit ? "Update header and line items." : "Issue a new customer invoice."}
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-1 flex-col overflow-hidden">
          <div className="flex-1 overflow-y-auto px-6 py-5">
            ...
            {isEdit && invoice && (
              <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
                <div className="rounded-md border border-gray-200 bg-gray-50 px-3 py-2">
                  <div className="text-[10px] uppercase tracking-wide text-gray-500">Grand total</div>
                  <div className="text-sm font-semibold text-gray-900">{formatCurrency(Number(invoice.grand_total))}</div>
                </div>
                <div className="rounded-md border border-gray-200 bg-gray-50 px-3 py-2">
                  <div className="text-[10px] uppercase tracking-wide text-gray-500">Paid</div>
                  <div className="text-sm font-semibold text-gray-900">{formatCurrency(Number(invoice.paid_amount))}</div>
                </div>
                <div className="rounded-md border border-gray-200 bg-gray-50 px-3 py-2">
                  <div className="text-[10px] uppercase tracking-wide text-gray-500">Balance</div>
                  <div className={`text-sm font-semibold ${Number(invoice.balance) > 0 ? "text-red-600" : "text-gray-900"}`}>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div>
                <label className="mb-1.5 block text-xs font-medium text-gray-700">Invoice date <span className="text-red-500">*</span></label>
