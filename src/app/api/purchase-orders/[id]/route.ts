...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({ error: "Unable to load purchase order." }, { status: 500 });
  }
}

/* ============================================================
   PUT  /api/purchase-orders/:id  (transaction)
============================================================ */
  ...
  try {
    ...
    if (!supplier_id || !order_date || !items.length) {
      return NextResponse.json(
        { error: "Supplier, order date and at least one line item are required." },
        { status: 400 }
      );
    }
...
export async function DELETE(
  ...
  try {
    ...
    await query("DELETE FROM purchase_orders WHERE id = ? AND company_id = ?", [id, companyId]);
